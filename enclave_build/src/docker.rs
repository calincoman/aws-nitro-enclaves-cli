...
impl std::fmt::Display for DockerError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            DockerError::BuildError => write!(f, "Failed to build EIF from docker image."),
            DockerError::InspectError => write!(f, "Failed to inspect docker image."),
            DockerError::PullError => write!(f, "Failed to pull docker image."),
            DockerError::RuntimeError => write!(f, ""),
            DockerError::TempfileError => write!(f, ""),
            DockerError::CredentialsError(msg) => write!(f, "{}", msg),
            DockerError::UnsupportedEntryPoint => write!(f, "")
        }
    }
}
...
impl ImageManager for DockerImageManager {
    ...
    fn pull_image(&mut self) -> Result<(), Box<dyn error::Error>> {
        let act = async {
                    ...
                    match item {
                        Ok(output) => {
                            ...
                            if let Some(err_msg) = msg.get("error") {
                                error!("{:?}", err_msg.clone());
                                break Err(DockerError::PullError.into());
                            } else {
                                info!("{}", msg);
                            }
                        }
    ...
    fn build_image(&self, dockerfile_dir: String) -> Result<(), Box<dyn error::Error>> {
        let act = async {
                ...
                if let Some(item) = stream.next().await {
                    match item {
                        Ok(output) => {
                            ...
                            if let Some(err_msg) = msg.get("error") {
                                error!("{:?}", err_msg.clone());
                                break Err(DockerError::BuildError.into());
                            } else {
                                info!("{}", msg);
                            }
                        }
    ...
    fn inspect_image(&mut self) -> Result<serde_json::Value, Box<dyn error::Error>> {
        let act = async {
            match self.docker.images().get(&self.docker_image).inspect().await {
                Ok(image) => Ok(json!(image)),
                Err(e) => {
                    error!("{:?}", e);
                    Err(DockerError::InspectError.into())
                }
            }
        };
    ...
    fn architecture(&mut self) -> Result<String, Box<dyn error::Error>> {
        let arch = async {
            match self.docker.images().get(&self.docker_image).inspect().await {
    ...
    fn load(&mut self) -> Result<(NamedTempFile, NamedTempFile), Box<dyn error::Error>> {
...
pub fn write_config(config: Vec<String>) -> Result<NamedTempFile, DockerError> {
